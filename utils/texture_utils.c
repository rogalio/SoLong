...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/01/12 18:24:22 by rogalio           #+#    #+#             */
/*   Updated: 2024/01/12 19:06:55 by rmouchel         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	load_all_textures(t_game *game, int num_files)
{
	...
	while (i < num_files)
	{
		if (load_texture(game->window.mlx, files[i],
				&game->window.texture[i]) != 0)
		{
...
{
	tex->img = mlx_xpm_file_to_image(mlx_ptr, file,
			&tex->width, &tex->line_length);
	if (tex->img == NULL)
	{
		write(1, "Error: Could not load texture\n", 30);
...
void	draw_texture(t_window *data, t_texture *texture, int x, int y)
{
	mlx_put_image_to_window(data->mlx, data->win,
		texture->img, x * TILE_SIZE, y * TILE_SIZE);
}
